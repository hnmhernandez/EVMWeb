...
function procesar() {
    new EVMFile(document.getElementById("fileEVM").files, function (evmResult) {
        ...
        new EVMFile(document.getElementById("fileEVM2").files, function (evmResult) {
            ...
            escribirEVM(evmTotal);
            arrayTime.push((mark_end-mark_start) * 1000)
        });
    });
}

function escribirEVM(evm){
    document.getElementById("status").innerHTML = "<center><h3>Results:</h3></center><p>EVs:" + evm.NEV + "\tOrder:" + evm.ABC + "\tDimension:" + evm.dim+"</p>";
    
    document.getElementById("status").innerHTML += "<ul>"
    for(var i=0; i< evm.NEV; i++){
        document.getElementById("status").innerHTML += "<li>("+evm.v[i].X + "," +evm.v[i].Y+","+evm.v[i].Z+")</li>";
    }
